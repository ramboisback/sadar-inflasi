...
async function startServer() {
  ...
  const writeAuditLog = (
    ...
    console.log(
      `[SIPID SECURITY AUDIT] [${timestamp}] [${status}] Actor: ${actor} | Action: ${action} | Details: ${details}`,
    );
  };
  ...
  const logFailedLoginAttempt = (ip: string) => {
    ...
    if (record.count >= 5) {
      ...
      writeAuditLog(
        ...
        ip,
        "WARNING",
        "Blocked repeated failing login attempts.",
      );
    }
  ...
  const verifyCryptographicToken = (
    token: string,
    ...
    try {
      ...
      if (
        clientSignature.length === expectedSignature.length &&
  ...
  const isInputUnsafe = (text: any): boolean => {
    ...
    return (
      maliciousPatterns.some((pattern) => pattern.test(text)) ||
      text.includes("<") ||
      text.includes(">")
    );
  };
  ...
  const authenticateToken = (
    ...
    if (!verified) {
      writeAuditLog(
        "EXPIRED_OR_FORGED_TOKEN",
        req.ip || "unknown",
        "WARNING",
        "Client supplied an invalid or modified token payload.",
      );
  ...
  function getCommoditiesSummary() {
    ...
    return dbObj.commodities.map((comm) => {
      ...
      const logs = dbObj.priceLogs
        .filter(
          (log) => log.commodity_id === comm.id && log.market_id === market.id,
        )
        .sort(
          (a, b) =>
      ...
      const hetViolationPercentage = isWarning
        ? ((latest_price - comm.het_price) / comm.het_price) * 100
        : 0;
      ...
      res.json({
        ...
        stats: {
          ...
          lastUpdated: commodities.reduce((acc, curr) => {
            ...
            return new Date(curr.latest_recorded_at).getTime() >
              new Date(acc).getTime()
              ? curr.latest_recorded_at
              : acc;
          }, ""),
        },
      });
    ...
    try {
      ...
      if (isInputUnsafe(username)) {
        ...
        writeAuditLog(
          ...
          "SUSPICIOUS_INPUT",
          "WARNING",
          `Suspicious blocklist characters inside login inputs from IP: ${ip}`,
        );
        return res
          .status(400)
          .json({ error: "Karakter input tidak valid deteksi." });
      }

      const user = dbObj.users.find(
      ...
      const legacyHashedInput = crypto
        .createHash("sha256")
        .update(password)
        .digest("hex");
      ...
      if (isLegacyMatch && !isSaltedMatch) {
        ...
        writeAuditLog(
          ...
          username,
          "SUCCESS",
          `User password hash upgraded in-place to salt structure for IP ${ip}`,
        );
      }
      ...
      writeAuditLog(
        ...
        user.username,
        "SUCCESS",
        `User authenticated and cryptographic session established from IP: ${ip}`,
      );
      ...
      writeAuditLog(
        ...
        "FAILURE",
        `Severe login error: ${e.message}`,
      );
      res
        .status(500)
        .json({ error: "Terjadi gangguan internal keamanan login." });
    }
  });
  ...
  app.post("/api/surveyor/log-price", authenticateToken, (req, res) => {
    try {
      ...
      if (!commodity_id || recorded_price === undefined || !recorded_at) {
        return res
          .status(400)
          .json({ error: "Parameter log-price tidak lengkap." });
      }
      ...
      if (isInputUnsafe(commodity_id) || isInputUnsafe(recorded_at)) {
        writeAuditLog(
          ...
          user.username,
          "WARNING",
          "Injection characters detected in dynamic parameters.",
        );
      ...
      if (isNaN(priceNum) || priceNum <= 0 || priceNum > 1000000) {
        // Limit max price logic to prevent buffer or database injection values
        return res.status(400).json({
          error: "Isian harga tidak valid atau di luar jangkauan regulasi.",
        });
      }
      ...
      writeAuditLog(
        ...
        user.username,
        "SUCCESS",
        `Updated price of '${comm.name}' to Rp ${priceNum} for date ${recorded_at}`,
      );
    ...
    try {
      ...
      if (user.role !== "admin") {
        writeAuditLog(
          ...
          user.username,
          "WARNING",
          "Access Denied: Non-administrator attempted to modify HET / Commodity parameters.",
        );
        return res
          .status(403)
          .json({ error: "Akses dibatasi hanya untuk Administrator." });
      }
      ...
      if (!id) {
        return res
          .status(400)
          .json({ error: "ID Komoditas wajib disertakan." });
      }
      ...
      if (isInputUnsafe(id) || isInputUnsafe(name) || isInputUnsafe(unit)) {
        writeAuditLog(
          ...
          user.username,
          "WARNING",
          "Blocked malicious HTML/script contents in parameters.",
        );
        return res.status(400).json({
          error: "Nilai masukan tidak valid atau melanggar kebijakan keamanan.",
        });
      }
      ...
      writeAuditLog(
        ...
        user.username,
        "SUCCESS",
        `Modified commodity HET details for '${oldName}': New HET Rp ${het_price || "N/A"}`,
      );
      ...
      writeAuditLog(
        ...
        (req as any).user?.username || "unknown",
        "FAILURE",
        `Failed update commodity details: ${e.message}`,
      );
      res
        .status(500)
        .json({ error: "Terjadi kegagalan memproses penyimpanan data HET." });
    }
  });
    ...
    try {
      ...
      if (user.role !== "admin") {
        writeAuditLog(
          ...
          user.username,
          "WARNING",
          "Access Denied: Non-administrator attempted to add new commodity.",
        );
        return res
          .status(403)
          .json({ error: "Akses dibatasi hanya untuk Administrator." });
      ...
      if (!name || !category || !unit || het_price === undefined) {
        return res
          .status(400)
          .json({ error: "Data komoditas baru tidak lengkap." });
      }

      // Input Validation against Injection / Stored XSS
      if (
        isInputUnsafe(name) ||
        isInputUnsafe(category) ||
        isInputUnsafe(unit)
      ) {
        writeAuditLog(
          "CREATE_COMMODITY",
          user.username,
          "WARNING",
          "Blocked malicious scripting or injection attempts in commodity fields.",
        );
        return res.status(400).json({
          error: "Data masukan mengandung karakter atau skrip berbahaya.",
        });
      }
      ...
      if (isNaN(priceNum) || priceNum <= 0) {
        return res
          .status(400)
          .json({ error: "HET harus berupa angka positif." });
      }
      ...
      writeAuditLog(
        ...
        user.username,
        "SUCCESS",
        `Created new commodity '${name}' under category '${category}' with HET ${priceNum}`,
      );
      ...
      writeAuditLog(
        ...
        (req as any).user?.username || "unknown",
        "FAILURE",
        `Failed to add commodity: ${e.message}`,
      );
      res.status(500).json({
        error: "Gagal memproses penambahan komoditas baru secara aman.",
      });
    }
  });
    ...
    try {
      ...
      if (user.role !== "admin") {
        writeAuditLog(
          ...
          user.username,
          "WARNING",
          `Access Denied: Non-administrator attempted to delete commodity ID: ${req.params.id}`,
        );
        return res
          .status(403)
          .json({ error: "Akses dibatasi hanya untuk Administrator." });
      ...
      if (isInputUnsafe(id)) {
        writeAuditLog(
          ...
          user.username,
          "WARNING",
          "Invalid parameter characters in commodity ID.",
        );
        return res
          .status(400)
          .json({ error: "Parameter tidak valid didefinisikan." });
      }
      ...
      dbObj.priceLogs = dbObj.priceLogs.filter(
        (log) => log.commodity_id !== id,
      );
      ...
      writeAuditLog(
        ...
        user.username,
        "SUCCESS",
        `Deleted commodity '${deletedName}' (ID: ${id}) and purged all price log histories.`,
      );
      ...
      writeAuditLog(
        ...
        (req as any).user?.username || "unknown",
        "FAILURE",
        `Failed to delete commodity: ${e.message}`,
      );
      res
        .status(500)
        .json({ error: "Gagal memproses penghapusan komoditas secara aman." });
    }
  });
...
3. **Rekomendasi Tindakan Cepat (Taktis)**: Ajukan minimal 3 langkah nyata TPID (Tim Pengendalian Inflasi Daerah) Bangka Barat (misal: Operasi Pasar Terbukan Sidak, subsidi biaya logistik melalui Pelabuhan Tanjung Kalian, Kerjasama Antar Daerah / KAD).`;
  ...
  app.listen(PORT, "127.0.0.1", () => {
    console.log(
      `SIPID Bangka Barat server boot running on: http://127.0.0.1:${PORT}`,
    );
  });
}
