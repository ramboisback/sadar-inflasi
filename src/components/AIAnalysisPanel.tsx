...
import { Sparkles, Loader2, AlertCircle, FileText, Send, HelpCircle } from 'lucide-react';

interface AIAnalysisPanelProps {
  onRefreshTrigger?: () => void;
}

export default function AIAnalysisPanel({ onRefreshTrigger }: AIAnalysisPanelProps) {
  ...
  const fetchAIAnalysis = async () => {
    ...
    try {
      ...
      setErrorMsg(`Terjadi kegagalan komunikasi dengan peladen: ${e?.message || e}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderFormattedAnalysis = (text: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (trimmed.startsWith('### ')) {
        return (
          <h5 key={idx} className="text-sm font-bold text-slate-900 mt-4 mb-2 first:mt-0 flex items-center gap-1.5 border-b pb-1 border-slate-100">
      ...
      if (trimmed.startsWith('## ')) {
        return (
          <h4 key={idx} className="text-base font-extrabold text-slate-900 mt-5 mb-2.5 flex items-center gap-2">
      ...
      if (trimmed.startsWith('# ')) {
        return (
          <h3 key={idx} className="text-lg font-extrabold text-blue-900 tracking-tight mt-6 mb-3 border-l-4 border-blue-600 pl-3 py-0.5">
            {trimmed.replace('# ', '')}
          </h3>
        );
      }
      if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
        ...
        return (
          <li key={idx} className="text-xs text-slate-700 ml-4 mb-1.5 leading-relaxed list-disc">
      ...
      if (trimmed.startsWith('> ')) {
        return (
          <blockquote key={idx} className="border-l-4 border-amber-500 bg-amber-50/60 text-xs text-amber-900 p-2.5 my-2.5 rounded-r-md leading-relaxed font-medium">
  ...
  return (
    <div id="ai-inflation-advisor" className="bg-[#111827] text-white rounded border border-[#1f2937] shadow-sm overflow-hidden">
      {/* Panel Header */}
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <span>{analysisText ? 'Evaluasi Ulang Data' : 'Jalankan Analisis AI'}</span>
            </>
          )}
        </button>
      </div>
      ...
      <div className="p-4 md:p-5 bg-gray-950 text-left">
        {errorMsg && (
          <div className="mb-4 p-3 bg-amber-500/10 border border-amber-500/25 text-amber-300 rounded text-xs leading-relaxed flex items-start gap-2.5">
            ...
            <div>
              <span className="font-bold block text-amber-200 mb-0.5">Catatan Sistem:</span>
              <p>{errorMsg}</p>
            </div>
          </div>
        )}
        ...
        {loading ? (
          <div className="py-16 flex flex-col items-center justify-center text-center">
            ...
            <span className="text-slate-200 font-semibold text-sm">Menghubungkan ke TPID Analis AI...</span>
            <p className="text-[11px] text-slate-400 mt-1.5 max-w-sm leading-relaxed">
              Gemini sedang meninjau database komoditi Pasar Induk Mentok, membandingkan ketetapan HET, menghitung momentum fluktuasi, dan merumuskan agenda intervensi pasar yang direkomendasikan...
            </p>
          </div>
        ) : analysisText ? (
          ...
          <div className="py-10 flex flex-col items-center justify-center text-center border border-dashed border-gray-800 rounded bg-gray-950/20">
            ...
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-widest">Laporan Intervensi Belum Dibuat</h4>
            <p className="text-[10px] text-slate-400 max-w-xs mt-1 leading-relaxed">
              Klik tombol di atas untuk memerintahkan AI menganalisis ketetapan HET pangan harian dan memberikan 3 rekomendasi taktis.
            </p>
            <div className="mt-4 flex gap-2.5 flex-wrap justify-center text-[10px] text-blue-400 font-medium">
              <span className="px-2 py-0.5 bg-white/5 rounded border border-[#1f2937]">• Evaluasi Log Harga</span>
              <span className="px-2 py-0.5 bg-white/5 rounded border border-[#1f2937]">• Alur Logistik Tanjung Kalian</span>
              <span className="px-2 py-0.5 bg-white/5 rounded border border-[#1f2937]">• Formula Subsidy Operasi Pasar</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="px-4 py-3 md:px-5 bg-gray-950 border-t border-[#1f2937] text-[10px] text-slate-400 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <span className="flex items-center gap-1">
          ...
          Rekomendasi bersifat saran akademis/teknis penunjang keputusan dinas.
        </span>
