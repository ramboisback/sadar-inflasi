...
import babarLogo from "./logo/babar.png";

export default function App() {
  const [activeTab, setActiveTab] = useState<"dashboard" | "portal">(
    "dashboard",
  );
  ...
  const handleExportWeeklyPDF = () => {
    ...
    doc.text(
      "DINAS KOPERASI, UMKM DAN PERINDUSTRIAN KABUPATEN BANGKA BARAT",
      105,
      23,
      { align: "center" },
    );
    doc.text(
      "Sekretariat Tim Pengendalian Inflasi Daerah (TPID) • Pasar Induk Mentok",
      105,
      27,
      { align: "center" },
    );
    ...
    doc.text(
      `Waktu Cetak: ${wibTimeStr || new Date().toLocaleString()}`,
      16,
      48,
    );
    doc.text(
      "Keperluan: Bahan Pembahasan Rapat Koordinasi Pengendalian Inflasi",
      16,
      52,
    );
    ...
    list.forEach((item) => {
      ...
      doc.text(item.het_price.toLocaleString("id-ID"), 126, y + 5, {
        align: "right",
      });
      ...
      doc.text((item.latest_price || 0).toLocaleString("id-ID"), 161, y + 5, {
        align: "right",
      });

      if (item.status === "UP") {
        doc.setTextColor(220, 38, 38);
        doc.text(`Naik (+${item.percentChange}%)`, 191, y + 5, {
          align: "right",
        });
      } else if (item.status === "DOWN") {
        doc.setTextColor(22, 163, 74);
        doc.text(`Turun (${item.percentChange}%)`, 191, y + 5, {
          align: "right",
        });
      } else {
    ...
    doc.text("Kepala Dinas Koperasi, UMKM, dan Perindustrian", 152, y + 4.5, {
      align: "center",
    });
    ...
    doc.save(
      `Laporan_Mingguan_Pangan_Mentok_${new Date().toISOString().split("T")[0]}.pdf`,
    );
  };
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setWibTimeStr(
        `${dayName}, ${day} ${monthName} ${year} - ${hrs}:${mins}:${secs} WIB`,
      );
    };
  ...
  const filteredCommodities =
    data?.commodities.filter((item) => {
      const matchesCategory =
        activeCategory === "Semua" || item.category === activeCategory;
      const matchesSearch = item.name
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
      const matchesHETFilter =
        !showSiagaHetOnly ||
        item.isWarning === true ||
        item.latest_price > (alertThresholds[item.id] || item.het_price);
      return matchesCategory && matchesSearch && matchesHETFilter;
    }) || [];

  const selectedCommodityDetails = data?.commodities.find(
    (c) => c.id === selectedCommodityId,
  );

  return (
    <div
      className={`min-h-screen flex flex-col transition-colors duration-250 ${isDarkMode ? "bg-[#090d16] text-slate-100" : "bg-[#F3F4F6] text-[#1A1A1A]"} font-sans antialiased`}
    >
      ...
      <header className="bg-[#111827] text-[#9ca3af] border-b border-[#1f2937] shadow-sm">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 md:py-5 flex flex-col md:flex-row md:items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-4">
            {/* Elegant Government Crest Emblem of Bangka Barat */}
            <div className="w-12 h-12 rounded flex items-center justify-center p-1 shrink-0">
              <img
                src={babarLogo}
                alt="Logo Bangka Barat"
            ...
            <div className="text-left">
              ...
              <h1 className="text-lg font-bold text-white tracking-tight leading-tight mt-0.5">
                SADAR <span className="text-yellow-500">INFLASI</span>{" "}
                <span className="text-blue-500">BANGKA BARAT</span>
              </h1>
              <p className="text-xs text-[#9ca3af] font-medium leading-relaxed uppercase">
                Sistem Informasi Pengendalian Inflasi Daerah • Titik Pantau
                Pasar Mentok
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-4 md:py-6">
        {loading && !data ? (
          <div className="py-24 flex flex-col items-center justify-center text-center">
            ...
            <span className="text-sm font-semibold text-slate-700 mt-4">
              Memuat data ...
            </span>
            <p className="text-xs text-slate-400 mt-1 max-w-xs leading-relaxed">
              Sistem sedang meresolusi riwayat log harga harian Pasar Induk
              Mentok.
            </p>
          </div>
        ) : activeTab === "portal" ? (
          ...
          <div className="space-y-5">
            ...
            <div
              className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 rounded shadow-sm divide-y sm:divide-y-0 sm:divide-x overflow-hidden border transition-all duration-200 ${isDarkMode ? "bg-[#111827] border-slate-800 divide-slate-800" : "bg-white border-gray-200 divide-gray-100"}`}
            >
              ...
              <div className="p-5 flex items-center justify-between">
                <div className="text-left">
                  ...
                  <span
                    className={`text-2xl font-bold font-mono mt-0.5 block ${isDarkMode ? "text-slate-100" : "text-gray-900"}`}
                  >
                ...
                <div
                  className={`p-2.5 rounded h-10 w-10 flex items-center justify-center shrink-0 border ${isDarkMode ? "bg-slate-800/40 border-slate-800 text-slate-400" : "bg-gray-50 border-gray-200 text-gray-500"}`}
                >
              ...
              <div className="p-5 flex items-center justify-between">
                <div className="text-left">
                  ...
                  <span
                    className={`text-2xl font-bold font-mono mt-0.5 block ${data?.stats.alertProductsCount && data.stats.alertProductsCount > 0 ? "text-red-500" : isDarkMode ? "text-slate-100" : "text-gray-900"}`}
                  >
                    {data?.stats.alertProductsCount || 0}
                  </span>
                  {data?.stats.alertProductsCount &&
                  data.stats.alertProductsCount > 0 ? (
                ...
                <div
                  className={`p-2.5 rounded border h-10 w-10 flex items-center justify-center shrink-0 ${
                    data?.stats.alertProductsCount &&
                    data.stats.alertProductsCount > 0
              ...
              <div className="p-5 flex items-center justify-between">
                <div className="text-left">
                  <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest block">
                    Lokasi Patokan Harga
                  </span>
                  <span
                    className={`text-sm font-bold mt-1 block font-sans ${isDarkMode ? "text-slate-100" : "text-gray-900"}`}
                  >
                    {data?.market.name || "Pasar Induk Mentok"}
                  </span>
                  <span
                    className={`text-[10px] font-medium block mt-1 leading-tight flex items-center gap-0.5 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
                  >
              ...
              <div className="p-5 flex items-center justify-between">
                <div className="text-left">
                  ...
                  <span
                    className={`text-sm font-bold mt-1 block leading-tight font-sans ${isDarkMode ? "text-slate-100" : "text-gray-900"}`}
                  >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
              ...
              <div className="lg:col-span-7 space-y-5">
                ...
                <div
                  className={`rounded border shadow-sm overflow-hidden p-4 space-y-3 transition-colors duration-200 ${isDarkMode ? "bg-[#111827] border-slate-800 text-slate-100" : "bg-white border-gray-200 text-gray-905"}`}
                >
                  <div
                    className={`flex items-center justify-between border-b pb-2 mb-1 ${isDarkMode ? "border-slate-800" : "border-gray-100"}`}
                  >
                    <div className="flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-blue-500" />
                      <h3
                        className={`text-xs font-bold uppercase tracking-widest ${isDarkMode ? "text-slate-100" : "text-[#1A1A1A]"}`}
                      >
                        Pemetaan Titik Pantau Pasar
                      </h3>
                    </div>
                    <span
                      className={`text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded border ${isDarkMode ? "bg-slate-900 border-slate-800 text-slate-400" : "bg-gray-50 border-gray-200 text-gray-400"}`}
                    >
                      Coords: {data?.market.latitude.toFixed(4)},{" "}
                      {data?.market.longitude.toFixed(4)}
                    </span>
                  </div>
                ...
                <div
                  className={`rounded border shadow-sm overflow-hidden p-4 space-y-4 transition-colors duration-200 ${isDarkMode ? "bg-[#111827] border-slate-800 text-slate-100" : "bg-white border-gray-200 text-gray-905"}`}
                >
                  {/* Title & Warning filters */}
                  <div
                    className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3 ${isDarkMode ? "border-slate-800" : "border-gray-101"}`}
                  >
                    <div className="text-left">
                      <h3
                        className={`text-xs font-bold uppercase tracking-widest ${isDarkMode ? "text-slate-100" : "text-gray-900"}`}
                      >
                        Indeks Harga Bahan Pangan Harian
                      </h3>
                      <p className="text-[11px] text-gray-500 font-medium">
                        Satuan per Kilogram (Kg) atau per Liter sesuai regulasi
                        pusat
                      </p>
                    </div>
                  ...
                  {/* Unified Table view */}
                  <div
                    className={`border rounded overflow-hidden shadow-sm transition-colors duration-200 ${isDarkMode ? "border-slate-800" : "border-gray-200"}`}
                  >
                    <table
                      className={`w-full text-left text-xs ${isDarkMode ? "text-slate-300" : "text-gray-600"}`}
                    >
                      <thead
                        className={`text-[11px] font-bold uppercase tracking-widest border-b transition-colors duration-200 ${isDarkMode ? "bg-slate-900/60 text-slate-400 border-slate-800" : "bg-gray-50 text-gray-500 border-gray-200"}`}
                      >
                        <tr>
                      ...
                      <tbody
                        className={`divide-y transition-colors duration-200 ${isDarkMode ? "divide-slate-800 bg-[#111827]" : "divide-gray-100 bg-white"}`}
                      >
                        {filteredCommodities.length === 0 ? (
                          <tr>
                            ...
                            >
                              Tidak ada komoditas ditemukan yang memenuhi
                              filter.
                            </td>
                          </tr>
                        ) : (
                              ...
                              >
                                <td className="px-4 py-3.5">
                                  <div className="flex items-center gap-1.5">
                                    <span
                                      className={`font-bold transition leading-tight ${isSelected ? "text-blue-600 font-extrabold" : "text-gray-900 group-hover:text-blue-600"}`}
                                    >
                                      {item.name}
                                    </span>
                                    {item.isWarning && (
                                      <span
                                        className="w-2 h-2 rounded-full bg-red-500 animate-ping inline-block"
                                        title="Melampaui HET pemerintah!"
                                      ></span>
                                    )}
                                  </div>
                                ...
                                <td className="px-4 py-3.5 text-right">
                                  <span
                                    className={`font-bold font-mono text-xs block ${item.isWarning ? "text-red-600" : isDarkMode ? "text-white" : "text-gray-900"}`}
                                  >
                                    Rp{" "}
                                    {item.latest_price?.toLocaleString("id-ID")}
                                  </span>
                                  {item.isWarning && (
                                    <span className="text-[9px] font-bold text-red-600 bg-red-50 px-1 border border-red-100 rounded inline-block mt-0.5">
                                      +{item.hetViolationPercentage}% di atas
                                      HET Limit
                                    </span>
                                  )}
                                </td>
                                  ...
                                  <div className="flex items-center justify-center gap-1">
                                    {item.status === "UP" ? (
                                      <span
                                        ...
                                        title="Harga Naik dibanding Kemarin"
                                      >
                                      ...
                                      <span
                                        className="text-green-600 font-bold flex items-center gap-0.5"
                                        title="Harga Turun dibanding Kemarin"
                                      >
                                      ...
                                      <span
                                        className="text-gray-400 font-medium flex items-center gap-0.5"
                                        title="Harga Stabil"
                                      >
                  ...
                  <div className="text-[10px] text-gray-400 leading-normal flex items-start gap-1.5 pt-1">
                    ...
                    <span>
                      Klik salah satu baris produk komoditi pangan di atas untuk
                      meneliti sebaran grafik riwayat harga dan analisis
                      fluktuasi secara visual pada panel kanan.
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-5 space-y-5">
                ...
                <div
                  className={`p-5 rounded border shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all duration-200 ${isDarkMode ? "bg-[#111827] border-slate-800 text-slate-100 shadow-2xl" : "bg-white border-slate-200 text-slate-900 shadow-sm"}`}
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h4
                        className={`text-xs font-extrabold uppercase tracking-widest ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}
                      >
                        Laporan Mingguan TPID
                      </h4>
                    </div>
                    <p
                      className={`text-[11px] leading-relaxed max-w-xs ${isDarkMode ? "text-slate-450" : "text-slate-900"}`}
                    >
                      Unduh ringkasan pergerakan harga bahan pokok minggu ini
                ...
                {selectedCommodityId && selectedCommodityDetails ? (
                  <div className="space-y-4">
                    {historyLoading ? (
                      <div
                        className={`p-12 rounded border flex flex-col items-center justify-center text-center h-[340px] transition-colors ${isDarkMode ? "bg-[#111827] border-slate-800" : "bg-white border-gray-200"}`}
                      >
                        <div className="w-8 h-8 border-4 border-gray-200 border-t-blue-600 rounded-full animate-spin"></div>
                        <span className="text-xs font-semibold text-gray-400 mt-4 leading-relaxed">
                          Menyusun riwayat pergerakan harga komoditas...
                        </span>
                      </div>
                    ) : (
                      <>
                        ...
                        <div
                          className={`p-4 rounded border transition-all duration-200 ${isDarkMode ? "bg-[#111827] border-slate-800 text-slate-100" : "bg-white border-slate-200 text-gray-905 shadow-sm"}`}
                        >
                          <div
                            className={`flex items-center gap-2 mb-2 border-b border-dashed pb-2 ${isDarkMode ? "border-slate-800" : "border-gray-100"}`}
                          >
                            ...
                            <h5 className="text-[10px] font-extrabold uppercase tracking-wider">
                              Konfigurasi Alarm Ambang Batas Alert
                            </h5>
                          </div>

                          <p
                            className={`text-[10px] leading-relaxed mb-3 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
                          >
                            Tetapkan ambang batas alarm personal Anda untuk{" "}
                            <strong className="font-bold">
                              {selectedCommodityDetails.name}
                            </strong>
                            . Jika harga harian melebihi nilai ambang ini, alarm
                            siaga visual merah akan menyala.
                          </p>
                          ...
                          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                            <div className="flex-1">
                              <div className="flex items-center justify-between text-[10px] font-mono mb-1 font-semibold text-slate-450">
                                <span>
                                  HET: Rp{" "}
                                  {selectedCommodityDetails.het_price.toLocaleString(
                                    "id-ID",
                                  )}
                                </span>
                                <span>
                                  Alert: Rp{" "}
                                  {(
                                    alertThresholds[selectedCommodityId] ||
                                    selectedCommodityDetails.het_price
                              ...
                              <input
                                type="range"
                                min={Math.round(
                                  selectedCommodityDetails.het_price * 0.5,
                                )}
                                max={Math.round(
                                  selectedCommodityDetails.het_price * 1.8,
                                )}
                                step={100}
                                value={
                                  alertThresholds[selectedCommodityId] ||
                  ...
                  <div
                    className={`p-8 rounded border text-center py-16 text-gray-400 flex flex-col items-center justify-center shadow-sm transition-colors ${isDarkMode ? "bg-[#111827] border-slate-800" : "bg-white border-gray-200"}`}
                  >
                    <BarChart2 className="w-12 h-12 stroke-[1] text-gray-300 mb-2.5" />
                    <h4 className="text-xs font-bold text-gray-600 uppercase tracking-widest leading-normal">
                      Grafik Harga Historis
                    </h4>
                    <p className="text-[10px] text-gray-400 mt-1 max-w-xs leading-relaxed">
                      Klik salah satu komoditas bahan pangan harian di sisi kiri
                      untuk memvisualisasikan grafik tren harga secara
                      interaktif.
                    </p>
                  </div>
                )}
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
          <div className="space-y-0.5">
            ...
            <p className="text-[10px] text-gray-400 font-mono font-medium max-w-xl leading-relaxed">
              TPID Kab. Bangka Barat • API v4.2.1-stable • VPS Mandiri
              Pemerintah Daerah Bangka Barat
            </p>
          </div>

          <div className="flex flex-col items-center md:items-end gap-1 font-mono text-[10px] text-gray-400">
            <span className="uppercase font-bold text-gray-500 tracking-wider">
              Sadar Inflasi Bangka Barat Versi 2.0
            </span>
            <span>Hak Cipta Dilindungi © 2026</span>
          </div>
        </div>
      </footer>
    </div>
